			...
			switch($action){
				...
				case "login_customer":
					if(isset($_POST['username']) && isset($_POST['password'])){
						...
						//echo $username;
						$password = $_POST['password'];
						$result = mysqli_query($connect,"select * from customer_login where (phone = '$username' AND password = '$password' AND verified = '1')");
				...
				break;
				case "otp_verify_forgot":
					if(isset($_POST['phone']) && isset($_POST['otp'])){
						$phone = $_POST['phone'];
						$otp = $_POST['otp'];
						$result = mysqli_query($connect,"update `customer_login` set verified = '1' where (`phone`='$phone' and `phone_verify`='$otp')");
						if($result){
							$status = "success";
							$message = "otp verified";
							$response = null;
				...
				case "passChange":
					if(isset($_POST['phone']) && isset($_POST['password'])){
						$phone = $_POST['phone'];
						$password = $_POST['password'];
						$result = mysqli_query($connect,"update customer_login set password='$password' where phone='$phone'");
						if($result){
							$status = "success";
						...
						$status = "failure";
						$message = "phone number or password not set";
						$response = null;
					}
				break;
