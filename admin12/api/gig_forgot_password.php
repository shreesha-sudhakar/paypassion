	...
	if($_SERVER['REQUEST_METHOD'] == "POST"){
		if (isset($_POST["action"])){
			...
			if(isset($_POST['phone'])){
				...
				switch ($action){
					case "PHONE":
					//getting the values from the customer care login table
					...
					if(mysqli_num_rows($phone_list)>0){
						...
						$phoned='91'.$phone;
						$coded=$rangen;
						mysqli_query($connect,"update gig_login set otp='$coded',verify='0' where phone='$phone'");
					...
					if(isset($_POST['otp'])){
						...
						$verified=mysqli_query($connect,"select verify,otp from gig_login where verify='0' and otp='$otp'");
						if(mysqli_num_rows($verified)>0){								
							$verify=$row['verify'];								
							mysqli_query($connect,"update login set `verify`=1 where otp='$otp'");
							$status = "success";
							$response = NULL;
							$message =  "Data is updated into the table.";
							} else { 
							//executes when data is not updated in login table
							$status = "failure";
							$response = NULL;
					...
					$update_password=mysqli_query($connect,"select password,phone from gig_login where phone='$phone'");
					if(mysqli_num_rows($update_password)>0){
						$password=$_POST['password'];
						mysqli_query($connect,"update gig_login set `password`='$password',verify='1' where phone='$phone'");
						$status = "success";
						$response = NULL;
						$message =  "Password is reset into the table.";
						} else { 
						//executes when password is not updated in login table
						$status = "failed";
						$response = NULL;
